...
export const areaTreeDetailResolver: ResolveFn<any> = (route) => {
  const router = inject(Router);
  const apiService = inject(AreaTreeService);
  const notificationService = inject(NzNotificationService);
  const translateService = inject(TranslateService);
  const store$ = inject(Store);
  const id = route.paramMap.get(RouteUtils.AreaTreePage.CityId.replace(':', ''));
  ...
  return apiService.getCityDetail(id).pipe(
    map((res) => {
