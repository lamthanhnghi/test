...
import { BANK_MENU_ITEMS } from '@shared/constants';
import { eContentStatuses, eViewMode } from '@shared/enums';
import { IBank } from '@shared/bank.model';
...
export class BankListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.banks.base' }));
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'banks') return parent;
