...
import { IBrand, IConfirmModal, IFilterBase, IMcMenuItem, IPagination, ISaleUnit } from '@shared/models';
...
export class BrandListComponent {
  ...
  constructor(
    ...
    const statuses = this.menuItems.find(i => i.id == "brands")?.children?.map((item) => item['queryParams']?.status?.toString()) || [];
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'brands') return parent;
