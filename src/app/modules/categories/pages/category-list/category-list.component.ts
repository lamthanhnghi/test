...
import { CATEGORY_MENU_ITEMS, GROUPS_MENU_ITEMS } from '@shared/constants/side-menu.constant';
import { RouteUtils } from '@shared/utils';
import { eContentStatuses, eViewMode } from '@shared/enums';
import { IFilterBase, IGetSellerRequestingProfileListPayload, IMcMenuItem, IProductCategory } from '@shared/models';
import { skip, Subject, takeUntil } from 'rxjs';
import { NzTableQueryParams } from 'ng-zorro-antd/table';
import { ActivatedRoute, Router } from '@angular/router';
import { Store } from '@ngrx/store';
import { AttributesActions } from '@shared/stores';
import { ProductCategoryService } from '@shared/services';
...
export class CategoryListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    const statuses = this.menuItems.find(i => i.id == "categories")?.children?.map((item) => item['queryParams']?.status?.toString()) || [];
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'categories') return parent;
