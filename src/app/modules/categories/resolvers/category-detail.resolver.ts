...
export const categoryDetailResolver: ResolveFn<any> = (route: ActivatedRouteSnapshot) => {
  const router = inject(Router);
  const apiService = inject(ProductCategoryService);
  ...
  return apiService.getDetail(id!).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.CategoriesPage.Base]).then();
        return undefined;
      }
      store$.dispatch(stopLoading());
