...
export const debtPolicyDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(DebtPolicyService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.DeptPoliciesPage.Base]).then();
      }
      store$.dispatch(stopLoading());
