...
export const deliveryFeePolicyDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(DeliveryFeePolicyService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.DeliveryFeePoliciesPage.Base]).then();
      }
      store$.dispatch(stopLoading());
