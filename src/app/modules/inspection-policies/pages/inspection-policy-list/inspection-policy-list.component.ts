...
import { IInspectionPolicyItem } from '@shared/inspection-policy.model';
import { InspectionPolicyService } from '@shared/inspection-policy.service';

@Component({
  selector: 'app-inspection-policy-list',
  templateUrl: './inspection-policy-list.component.html',
  styleUrl: './inspection-policy-list.component.scss',
})
export class InspectionPolicyListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.policies.inspection' }));
    ...
    const statuses =
      this.menuItems
        .find((i) => i.id == 'inspection-policies')!
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'inspection-policies') return parent;
