...
export const InspectionPolicyDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(InspectionPolicyService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.InspectionPoliciesPage.Base]).then();
      }
      store$.dispatch(stopLoading());
