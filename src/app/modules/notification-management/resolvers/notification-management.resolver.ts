...
export const notificationManagementResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(NotificationManagementService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.NotificationManagement.Base]).then();
      }
      store$.dispatch(stopLoading());
