...
export const OrganizationRegistrationPolicyDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(OrganizationRegistrationPolicyService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.OrganizationRegistrationPoliciesPage.Base]).then();
      }
      store$.dispatch(stopLoading());
