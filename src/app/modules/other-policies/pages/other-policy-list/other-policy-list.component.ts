...
import { IOtherPolicyItem } from '@shared/other-policy.model';

@Component({
  selector: 'app-other-policy-list',
  templateUrl: './other-policy-list.component.html',
  styleUrl: './other-policy-list.component.scss',
})
export class OtherPolicyListComponent {
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.policies.other' }));
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'other-policies') return parent;
