...
import { IPaymentPolicyItem } from '@shared/payment-policy.model';
import { PaymentPolicyService } from '@shared/payment-policy.service';

@Component({
  selector: 'app-payment-policy-list',
  templateUrl: './payment-policy-list.component.html',
  styleUrl: './payment-policy-list.component.scss',
})
export class PaymentPolicyListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.policies.payment' }));
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'payment-policies') return parent;
