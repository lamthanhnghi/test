...
export const PaymentPolicyDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(PaymentPolicyService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.PaymentPoliciesPage.Base]).then();
      }
      store$.dispatch(stopLoading());
