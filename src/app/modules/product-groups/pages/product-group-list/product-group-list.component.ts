...
import { eContentStatuses, eLayouts } from '@shared/enums';
import { IFilterBase, IGetSellerRequestingProfileListPayload, IMcMenuItem, IProductGroupItem } from '@shared/models';
...
export class ProductGroupListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    const statuses = this.menuItems.find(i => i.id == "groups")?.children?.map((item) => item['queryParams']?.status?.toString()) || [];
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'groups') return parent;
