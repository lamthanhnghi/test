...
import { IProgramPolicy } from '@shared/program-policy.model';

@Component({
  selector: 'app-program-policy-list',
  templateUrl: './program-policy-list.component.html',
  styleUrl: './program-policy-list.component.scss',
})
export class ProgramPolicyListComponent implements OnInit, OnDestroy {
  menuItems = POLICIES_MENU_ITEMS;
  // categories = MOCK_DATA_CATEGORIES;
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.policies.program' }));
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'program-policies') return parent;
