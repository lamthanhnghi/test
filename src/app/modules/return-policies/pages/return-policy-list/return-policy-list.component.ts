...
import { IReturnPolicyItem } from '@shared/return-policy.model';

@Component({
  selector: 'app-return-policy-list',
  templateUrl: './return-policy-list.component.html',
  styleUrl: './return-policy-list.component.scss',
})
export class ReturnPolicyListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.policies.return' }));
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'return-policies') return parent;
