...
export const saleUnitDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(SaleUnitService)
  ...
  return apiService.getDetail({
    ...
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.SaleUnitsPage.Base]).then();
        return undefined;
      }
      store$.dispatch(stopLoading());
