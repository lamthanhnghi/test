...
export const salesFeePolicyDetailResolver: ResolveFn<any> = (route, state) => {
  const router = inject(Router);
  const apiService = inject(SalesFeePolicyService);
  ...
  return apiService.getDetail(id).pipe(
    map((res) => {
      if (!res.result?.data) {
        ...
        router.navigate(['/', RouteUtils.SalesFeePoliciesPage.Base]).then();
      }
      store$.dispatch(stopLoading());
