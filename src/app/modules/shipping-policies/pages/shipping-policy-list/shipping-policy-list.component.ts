...
import { IShippingPolicyItem } from '@shared/shipping-policy.model';

@Component({
  selector: 'app-shipping-policy-list',
  templateUrl: './shipping-policy-list.component.html',
  styleUrl: './shipping-policy-list.component.scss',
})
export class ShippingPolicyListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.store$.dispatch(AttributesActions.setTitle({ title: 'menu.policies.shipping' }));
        ...
        next: (res) => {
          if (res.statusCode == 200) {
            this.menuItems = this.menuItems.map((parent) => {
              if (parent.id != 'shipping-policies') return parent;
