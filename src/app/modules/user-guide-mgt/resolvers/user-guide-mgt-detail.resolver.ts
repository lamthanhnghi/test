...
export const userGuideMgtDetailResolver: ResolveFn<any> = (route: ActivatedRouteSnapshot) => {
  const router = inject(Router);
  const apiService = inject(UserGuideService);
  ...
  return apiService.getDetail({ userGuideID: id! }).pipe(
    map((res) => {
      if (!res.result?.data) {
