...
export const userDetailResolver: ResolveFn<any> = (route: ActivatedRouteSnapshot) => {
  ...
  if (!id) {
    ...
    // return apiService.checkDraftExis t().pipe(
    //   tap((res) => {
  ...
  return apiService.getDetail({ adminID: id! }).pipe(
    map((res) => {
      if (!res.result) {
        ...
        router.navigate(['/', RouteUtils.UsersPage.Base]).then();
        return undefined;
      }
      store$.dispatch(stopLoading());
